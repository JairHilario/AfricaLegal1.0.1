...
function Header({ stats, temaAtual = "light" }) {
  const {
    total = 0,
    ativos = 0,
    inativos = 0,
  ...
  const vencidosBox = isDark
    ? "border-rose-700 bg-rose-900/30"
    : "border-rose-200 bg-rose-50";
  const vencidosTitle = "text-rose-600";
  const vencidosValue = isDark
    ? "text-lg font-semibold text-rose-300"
    : "text-lg font-semibold text-rose-700";

  const riscoBox = isDark
    ? "border-amber-700 bg-amber-900/30"
    : "border-amber-200 bg-amber-50";
  const riscoTitle = "text-amber-600";
  const riscoValue = isDark
  ...
  return (
    <header className={headerClasses}>
      <h1 className={titleClasses}>Gestão de Clientes</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 text-xs">
        ...
        <div className={`${baseTile} ${vencidosBox}`}>
          <h3 className={vencidosTitle}>Vencidos</h3>
          <p className={vencidosValue}>{vencidos}</p>
        </div>

        <div className={`${baseTile} ${riscoBox}`}>
          <h3 className={riscoTitle}>Em Risco</h3>
          <p className={riscoValue}>{emRisco}</p>
        </div>
