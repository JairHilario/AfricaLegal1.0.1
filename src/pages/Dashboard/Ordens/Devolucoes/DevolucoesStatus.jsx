...
function DevolucoesStatus({
  ...
  const validate = () => {
    ...
    if (!form.contaId) {
      newErrors.contaId = "Obrigatório.";
    } else if (form.contaId === "OUTRA" && !form.contaLivre.trim()) {
      newErrors.contaLivre = "Informe os dados da conta.";
    }
    if (!form.referencia.trim()) newErrors.referencia = "Obrigatório.";
    if (form.valor === "" || isNaN(form.valor)) newErrors.valor = "Obrigatório.";
    setErrors(newErrors);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!validate()) return;
    if (onAddDevolucao) onAddDevolucao(form);
    resetForm();
  };
    ...
    <>
      <div className={wrapper}>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              <span>{showAddForm ? "Fechar" : "Nova devolução"}</span>
            </button>
          </div>
          ...
          <div className="relative w-full md:w-1/2">
            ...
            <input
              type="text"
              placeholder="Buscar por cliente, referência, status..."
              className={inputSoft}
            />
          </div>
        </div>
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
          ...
          <div className={cardPendentes}>
            <h3 className="text-amber-400">Pendentes</h3>
            <p className="text-lg font-semibold">
              {pendentes.toLocaleString()}
            </p>
          </div>

          <div className={cardAprovadas}>
            <h3 className="text-emerald-400">Aprovadas</h3>
            <p className="text-lg font-semibold">
              {aprovadas.toLocaleString()}
            </p>
          </div>

          <div className={cardRejeitadas}>
            <h3 className="text-rose-400">Rejeitadas</h3>
            <p className="text-lg font-semibold">
              {rejeitadas.toLocaleString()}
            </p>
          </div>
        </div>
      </div>
      ...
      {showAddForm && (
        <div className={wrapper}>
          ...
          >
            ...
            <div>
              <label className={label}>
                Conta (empresa) <span className="text-rose-500">*</span>
              </label>
              <select
                name="contaId"
                value={form.contaId}
              ...
              {form.contaId === "OUTRA" && (
                <input
                  type="text"
                  name="contaLivre"
                  value={form.contaLivre}
                  onChange={handleChange}
                  placeholder="Banco / NIB / IBAN da conta"
                  className={`${inputSoft} mt-2`}
                />
              )}
              {errors.contaLivre && (
                <p className={errorText}>{errors.contaLivre}</p>
              )}
            </div>
...
export default DevolucoesStatus;
