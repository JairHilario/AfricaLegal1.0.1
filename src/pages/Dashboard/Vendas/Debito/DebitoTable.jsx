...
function DebitoTable({ debitos = [], temaAtual = "light" }) {
    ...
    <div className={containerClasses}>
      <div className="mb-3 flex items-center justify-between text-xs">
        ...
        <span className={isDark ? "text-slate-400" : "text-slate-500"}>
          Total: <strong>{debitos.length}</strong>
        </span>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-left text-xs md:text-sm">
          <thead className={headerRowClasses}>
            <tr>
              <th className="px-3 py-2">Data do Débito</th>
              <th className="px-3 py-2">Débito nº</th>
          ...
          <tbody>
            ...
            {debitos.map((d) => (
              ...
              >
                <td className={dateCell}>{d.data}</td>
                <td className={monoCell}>{d.referencia}</td>
                <td className={normalCell}>{d.cliente}</td>
                <td className={monoCell}>{d.facturaNumero}</td>
                <td className={moneyCell}>{formatMoney(d.valorSemIva)}</td>
                <td className={moneyCell}>{formatMoney(d.valorIva)}</td>
                <td className={moneyCell}>
                  {formatMoney(d.valorComIva ?? d.valor)}
                </td>
                <td className="px-3 py-2">
                  <span className={getStatusClasses(d.status)}>
                    {d.status}
                  </span>
                </td>
                <td className="px-3 py-2">
                  <button
                    ...
                    onClick={() =>
                      console.log("Ver débito", d.id || d.referencia)
                    }
                  >
                    Ver
                  </button>
