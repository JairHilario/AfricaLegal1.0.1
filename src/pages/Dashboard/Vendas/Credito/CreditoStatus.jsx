...
function CreditoStatus({ search, setSearch, onAddCredito, temaAtual = "light" }) {
  ...
  return (
    <>
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              <span>{showAddForm ? "Fechar" : "Novo crédito"}</span>
            </button>
          </div>
      ...
      {showAddForm && (
        <AddCreditoForm onAddCredito={onAddCredito} temaAtual={temaAtual} />
      )}
    </>
  );
}
...
function AddCreditoForm({ onAddCredito, temaAtual = "light" }) {
  const isDark = temaAtual === "dark";
  ...
  const [form, setForm] = useState({
    ...
    facturaId: "",
    facturaRef: "",
    referenciaNumero: gerarNumeroInicial(),
    localizacao: "Armazem",
    prazoVencimento: "",
    data: hojeIso,
    dataFim: "",
  ...
  const badgePrefix =
    ...
    (isDark ? "border-slate-700 bg-slate-800 text-slate-300" : "border-sky-100 bg-sky-100 text-slate-700");

  const badgeInput =
    "flex-1 rounded-r-md border px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-sky-300 " +
    (isDark ? "border-slate-700 bg-slate-800 text-slate-100 placeholder-slate-500" : "border-sky-100 bg-sky-50 text-slate-900 placeholder-slate-400");

  const tableWrapper = "mt-4 overflow-x-auto rounded-md border " + (isDark ? "border-slate-800" : "border-sky-100");
  ...
  useEffect(() => {
    // Busca Clientes (Garantindo que trate 404)
    fetch(`${API_BASE}/clients`)
      .then((r) => r.ok ? r.json() : [])
      .then((data) => setClientes(Array.isArray(data) ? data : []))
      .catch(() => setClientes([]));

    // Busca Termos (Garantindo que trate 404)
    fetch(`${API_BASE}/termos-pagamento`)
      .then((r) => r.ok ? r.json() : [])
      .then((data) => setTermos(Array.isArray(data) ? data : []))
      .catch(() => setTermos([]));

    // Busca Produtos (Garantindo que trate 404)
  ...
  const handleProdutoSelect = (e) => {
    ...
    const ivaPercent =
      produto.tipoImposto === "regime_normal" ? 17 : produto.tipoImposto === "cont_simplf" ? 5 : 0;
  ...
  const handleSubmit = () => {
    ...
    if (!form.prazoVencimento) { alert("Seleciona o prazo de vencimento."); return; }
    ...
    const novo = {
      ...
      referencia: `NC-${form.referenciaNumero}`,
      data: form.data,
  ...
  return (
    <div className={containerClasses}>
      <h2 className={titleClasses}>Nova Nota de Crédito</h2>
      ...
      <div className="grid md:grid-cols-4 gap-4 mb-4">
        ...
        <div>
          <label className={labelClass}>Factura Relacionada <span className="text-rose-500">*</span></label>
          <select value={form.facturaId} onChange={handleFacturaChange} className={selectNormal}>
            ...
            {Array.isArray(facturas) && facturas.map((f) => (
              <option key={f.id} value={f.id}>{`${f.referencia} - ${f.data} - ${f.valor} ${f.moeda}`}</option>
            ))}
          </select>
        </div>
        ...
        <div>
          ...
          <div className="flex">
            <span className={badgePrefix}>NC-</span>
            <input type="text" value={form.referenciaNumero} onChange={handleChange("referenciaNumero")} className={badgeInput} />
          </div>
        </div>

        <div>
          <label className={labelClass}>Localização <span className="text-rose-500">*</span></label>
          <select value={form.localizacao} onChange={handleChange("localizacao")} className={selectNormal}>
      ...
      <div className={tableWrapper}>
        <table className="min-w-full text-xs">
          ...
          <tbody>
            {items.map((item) => {
              ...
              return (
                <tr key={item.id} className={rowBorder}>
                  ...
                  <td className="px-2 py-1 text-right"><input type="number" value={item.quant} onChange={(e) => updateItem(item.id, "quant", Number(e.target.value) || 0)} className={cellInput + " text-right"} /></td>
                  <td className="px-2 py-1 text-right"><input type="number" step="0.01" value={item.preco} onChange={(e) => updateItem(item.id, "preco", Number(e.target.value) || 0)} className={cellInput + " text-right"} /></td>
                  <td className="px-2 py-1 text-right"><input type="number" value={item.ivaPercent} onChange={(e) => updateItem(item.id, "ivaPercent", Number(e.target.value) || 0)} className={cellInput + " text-right"} /></td>
                  <td className="px-2 py-1 text-right">{ivaValor.toFixed(2)}</td>
                  <td className="px-2 py-1 text-right"><input type="number" value={item.descontoPercent} onChange={(e) => updateItem(item.id, "descontoPercent", Number(e.target.value) || 0)} className={cellInput + " text-right"} /></td>
                  <td className="px-2 py-1 text-right">{total.toFixed(2)}</td>
                  <td className="px-2 py-1 text-center">
                    <button type="button" onClick={() => removeItem(item.id)} className={"inline-flex items-center rounded-md border px-2 py-1 text-[10px] " + (isDark ? "border-rose-700 bg-rose-900/40 text-rose-300 hover:bg-rose-800" : "border-rose-200 bg-rose-50 text-rose-700 hover:bg-rose-100")}>
                      Remover
                    </button>
                  </td>
                </tr>
            ...
            <tr>
              <td colSpan={8} className={"px-3 py-2 text-left text-xs cursor-pointer " + (isDark ? "text-rose-300" : "text-rose-600")} onClick={addItemManual}>
      ...
      <div className={totalsBox}>
        ...
        <div className="grid grid-cols-2">
          <div className={totalsLabel}>Total do Documento (MZN)</div>
          <div className={totalsValue + " font-semibold"}>{totalDocumento.toFixed(2)}</div>
        </div>
      </div>
